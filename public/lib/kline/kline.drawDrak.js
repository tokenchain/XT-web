...
VOLUMEIndicator.prototype.__construct = function() {
    ...
    this.addOutput(new RangeOutputExpr("MA",
        ...
        OutputStyle.Line,
        Theme.Color.Indicator3
    ));
    this.addOutput(new RangeOutputExpr("MA",
        new MaExpr(VOLUME, M2),
        OutputStyle.Line,
        Theme.Color.Indicator4
    ));
};
...
TableLayout.prototype.drawGrid = function(context) {
    ...
    context.fillRect(this._areas[0].getRight(), this.getTop(), GLOBAL_VAR.lineWidth, this.getHeight());
    var i, cnt = this._areas.length - 2;
    for (i = 0; i < cnt; i += 2)
        context.fillRect(this.getLeft(), this._areas[i].getBottom(), this.getWidth(), GLOBAL_VAR.lineWidth);
    if (!mgr.getCaptureMouseWheelDirectly()) {
        for (i = 0, cnt += 2; i < cnt; i += 2) {
            if (this._areas[i].isSelected()) {
                ...
                context.strokeRect(
                    this.getLeft() + GLOBAL_VAR.lineWidth/2, this.getTop() + GLOBAL_VAR.lineWidth/2,
                    this.getWidth() - GLOBAL_VAR.lineWidth, this.getHeight() - GLOBAL_VAR.lineWidth);
                break;
            }
        }
    }
...
DockableLayout.prototype.drawGrid = function(context) {
    ...
    for (var i in this._areas) {
        ...
        switch (area.getDockStyle()) {
            case ChartArea.DockStyle.Left:
                context.fillRect(area.getRight(), top, GLOBAL_VAR.lineWidth, bottom - top);
                left += area.getWidth();
                break;
            case ChartArea.DockStyle.Top:
                context.fillRect(left, area.getBottom(), right - left, GLOBAL_VAR.lineWidth);
                top += area.getHeight();
                break;
            case ChartArea.DockStyle.Right:
                context.fillRect(area.getLeft(), top, GLOBAL_VAR.lineWidth, bottom - top);
                right -= area.getWidth();
                break;
            case ChartArea.DockStyle.Bottom:
                context.fillRect(left, area.getTop(), right - left, GLOBAL_VAR.lineWidth);
...
Range.prototype.setPaddingTop = function(p) {
    //主线图的顶端距离
    this._paddingTop = p * GLOBAL_VAR.scale;
};
Range.prototype.setPaddingBottom = function(p) {
    //主线图的底边距离
    this._paddingBottom = p * GLOBAL_VAR.scale;
};
...
Theme.Color = {
    ...
    LineFillColorTop : theme_color_id++,
    LineFillColorBottom : theme_color_id++
};
Theme.Font = {
...
DarkTheme.prototype.__construct = function() {
    ...
    this._fonts = [];
    this._fonts[Theme.Font.Default] = "24px Arial";
};
...
LightTheme.prototype.__construct = function() {
    ...
    if(isLight){
        ...
        this._colors[Theme.Color.Cursor] = "#ddd";
        ...
        this._colors[Theme.Color.Text4] = "#757b88";
        ...
        this._colors = [];
        this._colors[Theme.Color.Positive] = "rgba(255,83,83,.7)";
        this._colors[Theme.Color.Negative] = "rgba(6,176,124,.7)";
        this._colors[Theme.Color.PositiveDark] = "#e83057";
        this._colors[Theme.Color.NegativeDark] = "#31c871";
        this._colors[Theme.Color.Unchanged] = "#fff";
        //非常难找的背景颜色
        this._colors[Theme.Color.Background] = "#27253f";
        this._colors[Theme.Color.Cursor] = "rgba(255,255,255,.3)"; //鼠标十字线颜色
        this._colors[Theme.Color.RangeMark] = "#4039D4";  //坐标轴最新价颜色
        this._colors[Theme.Color.LineFillColorTop] 		= "#312d5a";
        this._colors[Theme.Color.LineFillColorBottom] 	= "#27263d";
        this._colors[Theme.Color.Indicator0] = "#443b8d";  //均线颜色
        this._colors[Theme.Color.Indicator1] = "rgba(195,154,119,.7)";  //均线颜色
        this._colors[Theme.Color.Indicator2] = "rgba(81,159,187,.7)"; //均线颜色
        this._colors[Theme.Color.Indicator3] = "#6bf"; //均线颜色
        this._colors[Theme.Color.Indicator4] = "#a5cf81"; //均线颜色
        this._colors[Theme.Color.Indicator5] = "#e18b89"; //均线颜色
        this._colors[Theme.Color.Grid0] = "#333";
        this._colors[Theme.Color.Grid1] = "#312e4b";//k线跟交易量的分割颜色
        this._colors[Theme.Color.Grid2] = "#666";
        this._colors[Theme.Color.Grid3] = "#888";
        this._colors[Theme.Color.Grid4] = "#aaa";
        this._colors[Theme.Color.TextPositive] = "rgba(255,83,83,1)";
        this._colors[Theme.Color.TextNegative] = "rgba(6,176,124,1)";
        this._colors[Theme.Color.Text0] = "#2B2929";
        this._colors[Theme.Color.Text1] = "#666";
        this._colors[Theme.Color.Text2] = "rgba(255,255,255,.2)";//坐标轴数字颜色
        this._colors[Theme.Color.Text3] = "#fff"; //十字架数字颜色
        this._colors[Theme.Color.Text4] = "#fff";//参数颜色
        this._colors[Theme.Color.LineColorNormal] 		= "#a6a6a6";
        this._colors[Theme.Color.LineColorSelected] 	= "#ffffff";
        this._colors[Theme.Color.CircleColorFill] 		= "#000000";
        this._colors[Theme.Color.CircleColorStroke] 	= "#ffffff";
...
//划线窝点
Plotter.drawLines = function(context, area, left, points, theme,fillColor) {
    var i, cnt = points.length;

    context.beginPath();
    if(fillColor!=undefined){
        context.moveTo(0, area.getHeight());
    ...
    if(fillColor!=undefined){
        context.lineTo(points[i-1].x, area.getHeight());
        context.lineTo(0,area.getHeight());
        var grd=context.createLinearGradient(0,0,0,area.getBottom() + 1000);
        // fillColor

        grd.addColorStop(0,theme.getColor(Theme.Color.LineFillColorTop));
        grd.addColorStop(1,theme.getColor(Theme.Color.LineFillColorBottom));
...
Plotter.drawDashedLine = function(context, x1, y1, x2, y2, dashLen, dashSolid) {
    ...
    if (dY == 0) {
        ...
        for (var i = 0; i < count; i++) {
            context.rect(x1, y1, dashSolid, GLOBAL_VAR.lineWidth);
            x1 += dashLen;
        }
        context.fill();
    } else {
...
Plotter.createHorzDashedLine = function(context, x1, x2, y, dashLen, dashSolid) {
    ...
    var count = (dX / dashLen + 0.5) << 0;
    for (var i = 0; i < count; i++) {
        context.rect(x1, y, dashSolid, GLOBAL_VAR.lineWidth);
        x1 += dashLen;
    }
};
...
CGridPlotter.prototype.Draw = function(context) {
    ...
    context.beginPath();
    var dashLen = 4, dashSolid = GLOBAL_VAR.lineWidth;
    if (Plotter.isChrome)
    { dashLen = 4; dashSolid = GLOBAL_VAR.lineWidth; }
...
IndicatorPlotter.prototype.Draw = function (context) {
    ...
    // context.clearRect(0,0,area.getRight() - left,area.getHeight());
    context.lineWidth=1;
    for (n = 0; n < outCount; n++) {
        var x = center;
        var fillColor = "rgba(36,47,56,0.2)";
        out = indic.getOutputAt(n);
        if (out.getStyle() == OutputStyle.Line) {
            ...
            if (points.length > 0) {
                ...
                //这里划线
                Plotter.drawLines(context, area, left, points, theme, fillColor);
            }
        }
    }
    context.restore();
...
IndicatorInfoPlotter.prototype.Draw = function (context) {
    ...
    //隐藏成交量区坐上的提示信息
    // if(area._name == "frame0.k0.indic0"){
    //     return;
    // }
...
LastClosePlotter.prototype.Draw = function (context) {
    ...
    context.textBaseline = "middle";
    context.fillStyle = theme.getColor(Theme.Color.RangeMark);
    context.strokeStyle = theme.getColor("#fff");
    ...
    context.lineTo(left, y-18);
    context.lineTo(area.getRight()-1, y-18);
    context.lineTo(area.getRight()-1, y+18);
    context.lineTo(left, y+18);
    ...
    var mainArea = mgr.getArea("frame0.k0.indic0");
    context.strokeStyle = theme.getColor(Theme.Color.RangeMark);
    context.beginPath();
    context.moveTo(mainArea.getLeft(), y);
    context.lineTo(mainArea.getRight(), y);
    context.stroke();

    context.fillStyle="#FFF";
    context.fillText(String.fromFloat(v, ds.getDecimalDigits()) + " ", left + 3, y);
};
...
RangeSelectionPlotter.prototype.Draw = function(context) {
    ...
    context.textAlign="center";
    context.textBaseline="middle";
    context.fillStyle = theme.getColor(Theme.Color.Cursor);
    var digits = 2;
...
DrawPriceLinesPlotter.prototype.draw = function(context) {
    ...
    Plotter.drawLine(context, this.startPoint.x, this.startPoint.y, this.areaPos.right, this.startPoint.y);
    context.fillText(text.toFixed(2), this.startPoint.x + 2, this.startPoint.y - 30);
};
...
BandLinesPlotter.prototype.drawLinesAndInfo = function(context, startPoint, endPoint) {
    ...
    for (var i=0; i < this.fiboSequence.length; i++){
        ...
        context.fillText(text, this.areaPos.left + 2, stageY - 30);
    }
};
...
function on_size() {
    ...
    canvasGroupRect.w = tabBarRect.w * 2;
    canvasGroupRect.h = tabBarRect.y * 2 - toolPanelRect.y * 2;
    toolBar.css({
