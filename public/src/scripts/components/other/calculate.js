$(document).ready(function() {
  ...
  function checkLength(num) {
    if (num.toString().length > 7 && num.toString().length < 10) {
      ...
      $("#display").css("margin-top", "44px");
    } else if (num.toString().length > 10 && num.toString().length < 16) {
		$("#display").css("font-size", "24px");
		 $("#display").css("margin-top", "74px");
	}
  ...
  $('#calculator .displayBox').mousedown(
      function (event) {
        var isMove = true;
        var abs_x = event.pageX - $('#calculator').offset().left;
        var abs_y = event.pageY - $('#calculator').offset().top;
  ...
  $("body").on("click","#calculatorBtn", function () {
     $(this).toggleClass("on");
     $("#calculator").slideToggle(200);
  })
